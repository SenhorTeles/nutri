...
1. Consulta a tabela 'verificando' no Supabase buscando registros com status 'Subindo'
...
4. Se confirmado: atualiza status na tabela 'verificando' para 'Verificado'
                   atualiza status na tabela 'confrontofiscal' para 'OK'
5. Se falhou: atualiza status na tabela 'verificando' para 'Erro'
...
def buscar_pendentes():
    """Busca registros com status 'Subindo' na tabela verificando"""
    try:
        url = f"{SUPABASE_URL}/rest/v1/verificando?status=eq.Subindo&select=*"
...
def atualizar_status_verificando(registro_id, novo_status, obs_extra=""):
    """Atualiza o status de um registro na tabela verificando"""
    try:
        url = f"{SUPABASE_URL}/rest/v1/verificando?id=eq.{registro_id}"
        payload = {"status": novo_status}
        if obs_extra:
            payload["obs"] = obs_extra
        response = requests.patch(url, headers=HEADERS, json=payload, timeout=10)
        if response.status_code in [200, 204]:
            log(f"  [OK] Status do registro verificando #{registro_id} atualizado para '{novo_status}'")
            return True
        else:
            log(f"  [ERRO] Falha ao atualizar verificando #{registro_id}: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        log(f"  [ERRO] Conexão ao atualizar verificando #{registro_id}: {e}")
        return False


def atualizar_status_confronto(confronto_id, novo_status, user_nome=None, user_email=None):
    """Atualiza o status de um registro na tabela confrontofiscal e salva auditoria"""
    try:
        ...
        response = requests.patch(url, headers=HEADERS, json=payload, timeout=10)
        if response.status_code in [200, 204]:
            log(f"  [OK] Status do confrontofiscal #{confronto_id} atualizado para '{novo_status}'")
            return True
        else:
            log(f"  [ERRO] Falha ao atualizar confrontofiscal #{confronto_id}: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        log(f"  [ERRO] Conexão ao atualizar confrontofiscal #{confronto_id}: {e}")
        return False


def atualizar_confronto_com_dados(confronto_id, winthor_vltotal, winthor_vlicms, user_nome=None, user_email=None):
    """Atualiza o confrontofiscal com os dados FRESCOS do WinThor + auditoria de validação"""
    try:
        url = f"{SUPABASE_URL}/rest/v1/confrontofiscal?id=eq.{confronto_id}"
        payload = {
            "status": "Validado",
        ...
        response = requests.patch(url, headers=HEADERS, json=payload, timeout=10)
        if response.status_code in [200, 204]:
            log(f"  [OK] confrontofiscal #{confronto_id} atualizado com dados frescos do WinThor (Total:{winthor_vltotal}, ICMS:{winthor_vlicms})")
            return True
        else:
            log(f"  [ERRO] Falha ao atualizar confrontofiscal #{confronto_id}: {response.status_code} - {response.text}")
            return False
...
def remover_da_verificando(registro_id):
    """Remove o registro da tabela verificando após verificação concluída"""
    try:
        url = f"{SUPABASE_URL}/rest/v1/verificando?id=eq.{registro_id}"
        response = requests.delete(url, headers=HEADERS, timeout=10)
        if response.status_code in [200, 204]:
            log(f"  [OK] Registro verificando #{registro_id} removido da tabela")
            return True
        else:
            log(f"  [ERRO] Falha ao remover verificando #{registro_id}: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        log(f"  [ERRO] Conexão ao remover verificando #{registro_id}: {e}")
        return False
...
def get_cnpj_tomador(xml_doc):
    ...
    if not xml_doc:
        return ""
    try:
        # Remover namespaces e prefixos para simplificar a busca
        clean_xml = re.sub(r'\sxmlns="[^"]+"', '', xml_doc, count=1)
...
def processar_registro(registro):
    ...
    # 1. Filial NF vs Filial (verificação estática, não muda)
    if str(codfilialnf) != str(codfilial):
        msg = f"Filial diverge (Filial:{codfilial} != FilialNF:{codfilialnf})"
        log(f"  [BLOQUEADO] {msg}")
    ...
    try:
        ...
        if winthor_conferido == 'S':
            ...
            # Atualiza confrontofiscal com os dados frescos do WinThor
                ...
                sql_update = f"""
                    ...
                    WHERE n.numtransent IN ({bind_upd_clause})
                      ...
                      AND EXISTS (
                          ...
                          WHERE b.numtransent = n.numtransent AND b.especie = 'CT'
                      )
                """
...
def main():
    ...
    log("Monitorando tabela 'verificando' no Supabase...")
